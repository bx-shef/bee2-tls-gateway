...
if [[ -x "$BEE2CMD_BIN" ]]; then
  ...
  st_out="$("$BEE2CMD_BIN" st rng 2>&1)" \
    || die "самотест: генератор псевдослучайных чисел НЕ прошёл самотест (СТБ 34.101.47). Криптостек сломан, шлюз не стартует. Вывод: ${st_out}"
  log 'самотест криптостека: контрольные примеры и ДСЧ — OK'
else
  # Не фатально: образ может быть собран без bee2cmd (сборочная стадия, чужая сборка),
  # и тогда честнее сказать «не проверено», чем промолчать. Молчание здесь читалось бы
  # как «проверено и сошлось» — ровно та подмена, из-за которой проверки и умирают.
  log "ВНИМАНИЕ: самотест криптостека НЕ ВЫПОЛНЕН — нет $BEE2CMD_BIN. Это не отказ проверки, а её отсутствие."
fi
