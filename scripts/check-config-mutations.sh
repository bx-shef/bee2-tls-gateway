...
# только смоук job'а `image`, двадцать минут и вне PR-цикла. Единственная проверка проекта
# без мутации, то есть ровно та нить, которую по правилам проекта снимают первой.
#
# Мутации ниже делятся на две семьи, и вторая важнее первой. Снятый вызов заметен: контейнер
# перестаёт печатать свою строку. Смягчённый отказ НЕ заметен ничем — шлюз стартует, лог
# выглядит нормально, а криптография под платёжным трафиком сломана.

# Семья 1: вызов исчез. Мутации сформулированы как правдоподобное ослабление, а не как
# удаление строки: так проверяется ещё и то, что иголка привязана к коду достаточно точно.
mutate("проверка бинаря ослаблена с -x до -e (файл есть, но не исполняется)",
       lambda: sub(EP, r'\[\[ -x "\$BEE2CMD_BIN" \]\]', '[[ -e "$BEE2CMD_BIN" ]]'),
       "бинарь bee2cmd на месте — проверка на месте")

mutate("контрольные примеры алгоритмов больше не вызываются",
       lambda: sub(EP, r"st alg 2>&1", "st algs 2>&1"),
       "контрольные примеры алгоритмов (st alg) — проверка на месте")

mutate("самотест ДСЧ больше не вызывается",
       lambda: sub(EP, r"st rng 2>&1", "st rngs 2>&1"),
       "самотест ДСЧ (st rng) — проверка на месте")
...
mutate("опрос движка потерял -t — доступность больше не выясняется",
       lambda: sub(EP, r"openssl engine -t bee2evp", "openssl engine bee2evp"),
       "движок bee2evp опрашивается — проверка на месте")

mutate("проверка доступности движка сведена к упоминанию его имени",
       lambda: sub(EP, r'== \*available\*', "== *bee2evp*"),
       "движок bee2evp доступен — проверка на месте")
...
mutate("проверка криптонабора ослаблена до префикса DHE-BIGN",
       ...
       "рабочий стек предлагает криптонабор BTLS — проверка на месте")
...
# Запирает решение, принятое после ревью #18: путь к bee2cmd зашит, и переменной окружения
# для него нет. Переменная была бы не выключателем, а способом ПОДДЕЛАТЬ УСПЕХ — заглушка с
# `exit 0` печатает ту же строку «— OK», что и честный самотест.
mutate("путь к bee2cmd снова берётся из окружения (GW_BEE2CMD вернулась)",
       lambda: sub(EP, r"^BEE2CMD_BIN=/opt/btls/bin/bee2cmd$",
                   'BEE2CMD_BIN="${GW_BEE2CMD:-/opt/btls/bin/bee2cmd}"'),
       "путь к bee2cmd зашит")

# --- строка успеха как контракт с оператором ----------------------------------------
#
# ⚠ Эти две мутации сторожат ПРОТИВОПОЛОЖНЫЕ стороны одного расхождения, и вторая нужна
# потому, что первая проверка умеет проходить вхолостую: пропади из README пример лога —
# сравнивать станет нечего, и «строки совпадают» зазеленеет на пустом множестве.
mutate("код печатает не ту строку, которую README обещает оператору",
       lambda: sub(EP, r"log 'самотест ядра bee2: контрольные примеры и ДСЧ — OK'",
                   "log 'самотест ядра bee2: OK'"),
       "строки самотеста в README совпадают")

mutate("пример лога убран из README — сравнивать стало нечего",
       lambda: sub(RM, r"^\[crypto-gw\] самотест ядра bee2:[^\n]*\n"
                       r"\[crypto-gw\] самотест рабочего стека:[^\n]*\n", ""),
       "README показывает строки успеха самотеста")
