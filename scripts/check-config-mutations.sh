...
# Третья проба отличается ВЕТКОЙ гейта, а не исходом: подмена её копией первой оставляет
# шаг зелёным и на вид полным — три вызова, три отказа, — но ветка `-r` не исполняется.
mutate("проба нечитаемой связки подменена копией пробы пустой",
       lambda: sub(CI, r"bundle_case /tmp/ca-unreadable\.pem",
                   "bundle_case /tmp/ca-empty.pem"),
