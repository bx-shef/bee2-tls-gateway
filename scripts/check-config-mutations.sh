...
mutate("регулярка списка методов ослаблена — прошёл бы любой мусор",
       lambda: sub(EP, r'\[\[ "\$methods" =~ \^\[A-Z\]\+\(,\[A-Z\]\+\)\*\$ \]\]',
                   '[[ "$methods" =~ ^.*$ ]]'),
       "мусор вместо методов отвергается", ALLOW)
...
mutate("развод меток вернулся к одному суффиксу без перепроверки",
       lambda: sub(EP, r'^      label_base="\$label"$', '      label="${label}-1"; label_base="$label"'),
