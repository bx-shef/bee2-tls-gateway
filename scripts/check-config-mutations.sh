...
# ⚠ Добавлены 20.08.2026 (C6). Они несут новые сторожа реестра исполняемого, а README и
# docs мутировать нельзя (см. ниже) — без этих двух проверки 13 остались бы непокрытыми
# вовсе. Оба файла правятся редко и в тех же PR, что и код, так что требование чистоты на
...
# ⚠ Состав источников энтропии (E5, #123). Пять проверок, пять мутаций.
mutate("предупреждение об отсутствии физического источника снято",
       lambda: sub(EP, r'log "ВНИМАНИЕ: физического источника энтропии НЕТ',
                   'log "источников достаточно ('),
       "отсутствие физического источника энтропии не проходит молча")

mutate("состав источников печатается только в плохом случае",
       lambda: sub(EP, r'log "источники энтропии: \$\{_es_list\} — физический \(trng/trng2\) доступен"',
                   'true'),
       "состав источников энтропии печатается всегда")

mutate("нехватка физического источника стала ФАТАЛЬНОЙ (log→die)",
       lambda: sub(EP, r'log "ВНИМАНИЕ: физического источника энтропии НЕТ \(в списке только \$\{_es_list\}\)"',
                   'die "физического источника энтропии НЕТ"'),
       "нехватка физического источника НЕ фатальна")

mutate("st_out переприсвоен между st rng и чтением состава источников",
       lambda: sub(EP, r'log \x27самотест рабочего стека: движок bee2evp доступен, криптонабор BTLS предлагается — OK\x27',
                   'st_out="переприсвоено"\nlog \x27самотест рабочего стека: движок bee2evp доступен, криптонабор BTLS предлагается — OK\x27'),
       "между снятием st rng и чтением состава источников st_out не переприсваивается")
...
#   «перечень не продублирован в PROCESS/CERTIFICATION» — мутационного покрытия НЕ имеют.
#   Названо здесь, чтобы это не выглядело недосмотром.
mutate("поведенческая сверка реестра с образом выпилена из CI",
       lambda: sub(CI, r"scripts/runtime-inventory\.py README\.md files",
                   "scripts/runtime-inventory.py README.md nothing"),
       "реестр сверяется с ОБРАЗОМ, а не только описан")

mutate("парсер реестра перестал находить таблицы — реестр стал пустым",
       lambda: sub(RI, r'FILES_HEADING = "### Файлы, положенные нами"',
                   'FILES_HEADING = "### Такого заголовка нет"'),
       "реестр исполняемого есть в README и разбирается")

# ⚠ Исключительные ситуации: лимит запросов и связка корней (критерий 6.4.7, #116).
#   Мутируем шаблон, entrypoint и ci.yml. Таблица §6.8 живёт в docs, а docs в MUTABLE не
#   входят, поэтому «код в документе разошёлся с конфигом» ловится с одной стороны —
#   мутацией шаблона. Со стороны документа сторожа нет; названо здесь, чтобы это не
#   выглядело недосмотром, и держится правилом «документация правится в том же PR».
mutate("код отказа по лимиту в конфиге разошёлся с таблицей §6.8",
       lambda: sub(TPL, r"limit_req_status 429;", "limit_req_status 503;"),
       "код отказа по лимиту совпадает")

# Возврат штатного лимита обнуляет пробу целиком: 10 r/s с burst серией из пяти запросов
# не пробить, и смоук зеленел бы, ничего не проверяя. Мутация заодно показывает, зачем в
# проверке запрет цифры следом: подстрока `GW_RATE=1` совпала бы и с `GW_RATE=10`.
mutate("проба лимита вернулась к штатному GW_RATE — серия его не пробьёт",
       lambda: sub(CI, r"-e GW_RATE=1 ", "-e GW_RATE=10 "),
       "лимит ПРОБИВАЕТСЯ смоуком")

# ⚠ Смягчение FATAL до предупреждения — самая дорогая из правок этого гейта: контейнер
# поднимется, healthcheck позеленеет, а каждый запрос к банку получит 502. Отказ станет
# тихим ровно там, где обязан быть громким.
mutate("нечитаемая связка стала предупреждением, а не отказом старта",
       lambda: sub(EP, r'\[\[ -r "\$GW_CA_BUNDLE" \]\] \|\| die',
                   '[[ -r "$GW_CA_BUNDLE" ]] || log'),
       "гейт «связка не читается» роняет старт")

mutate("связка без сертификатов стала предупреждением, а не отказом старта",
       lambda: sub(EP, r'\[\[ "\$cert_count" -ge 1 \]\] \|\| die',
                   '[[ "$cert_count" -ge 1 ]] || log'),
       "гейт «в связке нет сертификатов» роняет старт")

# Третья проба отличается ВЕТВЬЮ гейта, а не исходом: подмена её копией первой оставляет
# шаг зелёным и на вид полным — три вызова, три отказа, — но ветвь `-r` не исполняется.
mutate("проба отсутствующей связки подменена копией пробы пустой",
       lambda: sub(CI, r"bundle_case отсутствующая",
                   "bundle_case пустая"),
       "отказ старта по связке проверяется поведением")

# ⚠ Молчание внешней системы → 504 (#144). Таблица §6.8 живёт в docs, а docs в MUTABLE не
#   входят, поэтому две проверки со стороны документа («исход назван ровно один раз» и
#   «таблица обещает 504») мутационного покрытия НЕ имеют. Названо здесь, чтобы это не
#   выглядело недосмотром; держится правилом «документация правится в том же PR».
mutate("таймаут рукопожатия захардкожен в шаблоне мимо переменной",
       lambda: sub(TPL, r"proxy_connect_timeout \$\{GW_CONNECT_TIMEOUT\};",
                   "proxy_connect_timeout 15s;"),
       "таймаут рукопожатия с внешней системой берётся из GW_CONNECT_TIMEOUT")

# Возврат штатного таймаута обнуляет ВРЕМЕННУЮ часть утверждения: 504 всё равно придёт,
# но уже нельзя будет сказать, какой из двух таймаутов его дал, — а именно это и было
# главным вопросом задачи.
mutate("проба вернулась к длинному таймауту — какой сработал, станет неизвестно",
       lambda: sub(CI, r"-e GW_CONNECT_TIMEOUT=2s", "-e GW_CONNECT_TIMEOUT=20s"),
       "молчание внешней системы ПРОВЕРЯЕТСЯ пробой")

# Контраст выпилен подменой второй строки журнала копией первой: шаг остаётся зелёным и
# на вид полным, но перестаёт отличать молчание от отказа соединения.
mutate("контраст 504 против 502 подменён дублем 504",
       lambda: sub(CI, r"route=open-banking-v1-0 \.\*status=502",
                   "route=open-banking-v1-0 .*status=504"),
       "проба различает молчание (504) и отказ соединения (502)")

# ⚠ Шестая мутация заведена по замечанию панели (тестировщик): проверка «код отказа задан
# в шаблоне» была живой, но её живость никогда не доказывалась прогоном — а в этом проекте
# недоказанная проверка и мёртвая неотличимы.
mutate("директива кода отказа по лимиту удалена из шаблона",
       lambda: sub(TPL, r"^    limit_req_status 429;$", "    # limit_req_status 429;"),
       "код отказа по лимиту задан в шаблоне")

# ⚠ Паспорт сборки (МИ.10165 п. 6.1.3, C4.1). Мутируем Dockerfile и entrypoint.sh;
#   README в MUTABLE намеренно не входит (решение ревью #45, разбор в шапке списка),
#   поэтому две проверки README — «паспорт описан» и «не выдаёт за воспроизводимость» —
#   мутационного покрытия НЕ имеют. Названо здесь, чтобы это не выглядело недосмотром:
#   их держит правило «документация правится в том же PR, что и код».
mutate("генерация паспорта сборки удалена целиком",
       lambda: sub(DF, r'echo "ПАСПОРТ СБОРКИ crypto-gw"', 'echo "нет паспорта"'),
       "паспорт сборки собирается в Dockerfile")

mutate("из паспорта убрана версия компилятора — п. 6.1.3 назван прямо",
       lambda: sub(DF, r'echo "компилятор:', 'echo "сборщик:'),
       "паспорт несёт: версия компилятора")

# ⚠ Разрыв «поле снято, но в паспорт не попало»: сборочная стадия по-прежнему пишет
#   /build-info, все проверки полей зелёные — а компилятора в паспорте нет.
mutate("перенос /build-info в паспорт убран — поля собраны и потеряны",
       lambda: sub(DF, r'cat /tmp/bi-toolchain\.txt;', 'true;'),
       "снятое в сборочной стадии переносится в паспорт")

mutate("из паспорта убрана фактическая версия OpenSSL",
       lambda: sub(DF, r'OpenSSL \(факт\):', 'OpenSSL (заявлен):'),
       "паспорт несёт: фактическая версия OpenSSL")

mutate("гейт полноты паспорта снят — зависимость выпадет молча",
       lambda: sub(DF, r'ПАСПОРТ НЕПОЛОН', 'паспорт неполон, но это ничего'),
       "полнота паспорта сторожится гейтом сборки")

mutate("entrypoint перестал печатать паспорт при старте",
       lambda: sub(EP, r'log "паспорт сборки: \$BUILD_MANIFEST"', 'true'),
       "entrypoint печатает паспорт при старте")
