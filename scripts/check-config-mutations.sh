...
# --- расшифрованное тело не ложится на диск ---------------------------------------
# ⚠ Две мутации, а не одна, и вторая важнее первой. Снятая директива — случай очевидный;
# случай, ради которого проверка сверяет ЗНАЧЕНИЕ, — директива на месте с ненулевым
# размером: конфиг читается «ограничение стоит», а на диск при этом ложится мегабайт
# расшифрованной выписки. Проверка на вхождение подстроки прошла бы её зелёной.
mutate("proxy_max_temp_file_size удалён — вернулось умолчание 1024m",
       lambda: sub(TPL, r"\n\s*proxy_max_temp_file_size [^;]+;", ""),
       "расшифрованное тело не буферизуется на диск")

mutate("proxy_max_temp_file_size ненулевой — файл на диске всё равно создаётся",
