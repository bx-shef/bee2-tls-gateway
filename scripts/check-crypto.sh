...
# Здесь же виден корень issue #8: источник timer поднимает поток-счётчик,
# который крутится до конца процесса. Если физический источник (trng) здоров,
# добор энтропии таймером не нужен — и это проверяемо, а не на словах.
...
if [ -n "$BEE2CMD_BIN" ]; then
  ...
  if printf '%s' "$es_out" | grep -q 'Health2 (there is a healthy physical source): +'; then
    ok "физический источник (trng) доступен — busy-loop таймера не обязателен, см. #8"
  else
    skip "физического источника нет: на этой машине timer действительно нужен (#8)"
  fi
else
  skip "нет bee2cmd"
fi
