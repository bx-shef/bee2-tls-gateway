...
# Здесь мы проверяем не то, что самотест правильно считает (это не текстовый вопрос), а то,
# что он ВЫЗЫВАЕТСЯ и что его провал РОНЯЕТ контейнер. Именно вторая половина хрупкая:
# смягчить `die` до `log` — правка в четыре символа, выглядящая как снижение шума, после
# которой шлюз обслуживает платёжный трафик со сломанной криптографией и бодро об этом
# сообщает. Разбор — README § «Самотестирование криптостека».


def logical_line(text, needle):
    """Логическая строка bash с needle: до первого перевода строки без `\\` на конце.
...
def selftest_gate(label, needle):
    """Вызов самотеста на месте, и его провал заканчивается die, а не log."""
    line = logical_line(entrypoint_code, needle)
    check(f"самотест: {label} — проверка на месте", line is not None,
          f"в entrypoint.sh не найдено «{needle}»")
    if line is None:
        return
    # `die` ищем как отдельное слово: `|| die "…"`. Слово могло бы встретиться внутри
    # текста сообщения, но сообщение принадлежит самому die, так что ложного зелёного тут
    # нет — а вот `log` вместо `die` регулярка отличает надёжно.
    check(f"самотест: провал «{label}» роняет контейнер",
          re.search(r"\|\|\s*die\b", line) is not None,
          f"нет `|| die` в операторе: {' '.join(line.split())[:140]}")
...
#
# ⚠ Иголки подобраны так, чтобы совпадать ТОЛЬКО с кодом и никогда — с текстом сообщения
# внутри `die`. Иначе мутация, снявшая сам вызов, находилась бы по слову из сообщения
# соседней строки, и проверка «на месте» осталась бы зелёной. Так, `st alg` без `2>&1`
# совпал бы и с `st algs`, а имя криптонабора встречается в файле дважды — второй раз как
# раз в тексте отказа.
selftest_gate("бинарь bee2cmd на месте", '-x "$BEE2CMD_BIN"')
selftest_gate("контрольные примеры алгоритмов (st alg)", "st alg 2>&1")
selftest_gate("самотест ДСЧ (st rng)", "st rng 2>&1")
selftest_gate("движок bee2evp опрашивается", "openssl engine -t bee2evp")
selftest_gate("движок bee2evp доступен", "*available*")
selftest_gate("рабочий стек предлагает криптонабор BTLS",
              "grep -q 'DHE-BIGN-WITH-BELT-CTR-MAC-HBELT'")

# ⚠ Путь к bee2cmd обязан быть литералом. Здесь была переменная `GW_BEE2CMD` — ради теста
# «провал самотеста роняет контейнер», — и ревью показало, чем она была на самом деле: не
# выключателем, а способом ПОДДЕЛАТЬ УСПЕХ. Заглушка с `exit 0` печатала бы ту же строку
# «— OK», неотличимую от честной, а опечатка в пути тихо переводила бы фатальную проверку в
# предупреждение. Проверка, отменяемая переменной окружения, на платёжном пути не проверка.
bin_assign = re.search(r"^BEE2CMD_BIN=(.+)$", entrypoint_code, re.M)
if not bin_assign:
    check("путь к bee2cmd задан", False, "присваивание BEE2CMD_BIN= не найдено")
else:
    value = bin_assign.group(1).strip()
    check("путь к bee2cmd зашит, а не берётся из окружения",
          "$" not in value and value.strip('"\'').startswith("/"),
          f"значение подставляется извне: {value}")

# ⚠ Строка успеха — часть контракта с оператором, а не украшение лога. README показывает
# её как признак того, что самотест прошёл; если код печатает другое, оператор ищет в логе
# то, чего там нет, и делает вывод, что самотест не отработал. Эта проверка написана
# потому, что расхождение УЖЕ БЫЛО: README обещал «самотест криптостека: …», код печатал
# «самотест ядра bee2: …», и после #38 строк стало две вместо одной.
section = re.search(r"^## Самотестирование криптостека\n(.*?)(?=^## )", readme, re.M | re.S)
if not section:
    check("README: раздел про самотест найден", False, "нет раздела «## Самотестирование криптостека»")
else:
    promised = re.findall(r"^\[crypto-gw\] (.+)$", section.group(1), re.M)
    check("README показывает строки успеха самотеста", bool(promised),
          "в разделе нет ни одной строки вида [crypto-gw] …")
    # Сравниваем с тем, что реально печатает `log`, — литералом, без интерполяции: строки
    # самотеста статические, и в этом их удобство как признака.
    logged = set(re.findall(r"""^\s*log\s+'([^']+)'""", entrypoint_code, re.M))
    drifted = [p for p in promised if p not in logged]
    check("строки самотеста в README совпадают с тем, что печатает entrypoint.sh",
          not drifted, f"README обещает, а код не печатает: {drifted}")
