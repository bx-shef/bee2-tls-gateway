...
#   - extended_master_secret (#80, A1; обязательное требование СТБ 34.101.90-2025
#     приложение Б, действует с 01.07.2026) предлагается и
...
# ---- объявленные криптонаборы: один список на двух потребителей ---------------------
# ⚠ Список поднят сюда из матрицы 21.08.2026 (#132) и НЕ продублирован: по нему идёт и
# EMS-проба ниже, и сама матрица. Две копии разъехались бы — добавили бы набор в матрицу,
# а EMS остался бы проверенным на прежних четырёх.
# Это ровно те наборы, которые изделие ПРЕДЛАГАЕТ (`proxy_ssl_ciphers` шаблона). Стек
# умеет и PSK-наборы 0xFF19–0xFF1C, но изделие их не предлагает, и решение по ним не
# принято — A7 (#98). Пока оно не принято, «все реализованные криптонаборы» в смысле
# МИ.10165 п. 5.4 для нас — эти четыре.
declare -A EXPECT=(
...
# правит. Проба дешёвая — s_client без подъёма шлюзов.
# Норма: СТБ 34.101.90-2025 прил. Б (обязательное) — «использование расширения
# extended_master_secret является дополнительным обязательным требованием СТБ 34.101.65».
# Сообщение ОАЦ от 12.05.2025 — подтверждение, не основание;
...
# ⚠ Расширено 21.08.2026 (#132) с ОДНОЙ клетки на всю объявленную матрицу.
# МИ.10165 п. 5.4: «проверки… выполняются для ВСЕХ реализованных криптонаборов и всех
# поддерживаемых сторон и уровней стойкости». Прежде EMS снимался на btls256/8443/0xFF15
# — одной клетке из двенадцати, — и этого хватило бы, чтобы пропустить набор, на котором
# расширение не согласуется. Сторона у изделия одна (клиент), уровней стойкости три,
# объявленных наборов четыре: двенадцать проб. Шлюзы не поднимаются, это s_client.
ems_cells=0
for s in 256 384 512; do
  for port in "${!EXPECT[@]}"; do
    suite="${EXPECT[$port]}"
    ems=$(s_probe "btls-stand_s$s" "$port" -tlsextdebug -tls1_2 -cipher "$suite")
    grep -qi 'server extension "extended master secret"' <<<"$ems" \
      || { ems_dump "$ems"; die "btls$s/$suite: эталон не вернул extended_master_secret в ServerHello — RFC 7627 не согласован (блокер испытаний, #80)"; }
    grep -q 'Extended master secret: yes' <<<"$ems" \
      || { ems_dump "$ems"; die "btls$s/$suite: расширение вернулось, но в ключевой материал не вошло (#80)"; }
    ems_cells=$((ems_cells + 1))
  done
done
[ "$ems_cells" -eq 12 ] \
  || die "EMS снят не со всей матрицы: клеток $ems_cells, ожидалось 12 (3 уровня × 4 набора)"
...
# Негатив снимается на ОДНОЙ клетке, и это осознанно: он доказывает не свойство набора,
# а то, что сами грепы выше умеют краснеть. Двенадцать одинаковых негативов проверяли бы
...
log "ok   extended_master_secret согласуется на ВСЕХ $ems_cells клетках (3 уровня × 4 объявленных набора); проба краснеет, если -no_ems не даёт «no»"
