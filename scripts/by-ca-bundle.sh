...
MARKER='.by-ca-bundle-workdir'
if [[ -z "$OUT" ]]; then
  OUT="$(mktemp -d "${TMPDIR:-/tmp}/by-ca.XXXXXXXX")" || { echo "не могу создать временный каталог" >&2; exit 1; }
  : > "$OUT/$MARKER"
else
  # ⚠ ЧУЖОЙ КАТАЛОГ НЕ ЧИСТИМ — И ЭТО НЕ ПЕРЕСТРАХОВКА. Первая редакция этой правки делала
  # безусловный `rm -f "$OUT"/*.pem *.cer`, и ревью воспроизвело последствие запуском:
  # `--out ca` СТИРАЕТ ca/bank-leaf-reference.pem (эталон гейта сборки и check-ca.sh) и
  # перезаписывает боевую связку. Атакующий для этого не нужен — достаточно оператора,
  # который прочитал в шапке «производит ca/gossuok-bundle.pem» и указал очевидный путь.
  # Поэтому чистим ТОЛЬКО каталог, который создали сами: признак — файл-маркер.
  if [[ -e "$OUT" ]]; then
    if [[ ! -d "$OUT" ]]; then
      echo "--out: это не каталог: $OUT" >&2; exit 2
    fi
    if [[ ! -e "$OUT/$MARKER" ]] && [[ -n "$(ls -A "$OUT" 2>/dev/null)" ]]; then
      echo "--out: каталог $OUT не пуст и создан не этим скриптом — своё содержимое он не тронет." >&2
      echo "       Укажите пустой или несуществующий каталог; результат всё равно копируется в ca/ руками." >&2
      exit 2
    fi
...
chmod 700 "$OUT" || { echo "не удалось закрыть права на $OUT" >&2; exit 1; }
...
if [[ $OSSL_EXPLICIT -eq 1 ]]; then
  ...
  # Обе раскладки, как в bee2evp-probe.sh: установленная (build/local/bin) и внутридеревная
  # (build/openssl/apps). Проверка только первой давала «не найден пропатченный openssl —
  # сначала прогоните bee2evp-probe.sh» тому, кто его как раз прогнал.
  found="$(find "$PROBE_WORK" -type f -name openssl -perm -u+x -user "$(id -u)" 2>/dev/null | grep -E '/local/bin/openssl$' | head -1)"
  [[ -n "$found" ]] || found="$(find "$PROBE_WORK" -type f -name openssl -perm -u+x -user "$(id -u)" 2>/dev/null | grep -E '/apps/openssl$' | head -1)"
  if [[ -n "$found" ]]; then
    OSSL="$found"
    LIBDIR="$(dirname "$(dirname "$OSSL")")/lib"
    [[ -d "$LIBDIR" ]] || LIBDIR="$(dirname "$(dirname "$OSSL")")"
...
ambiguous=0
walked=0
# ⚠ ОБХОД ИДЁТ ПО ВСЕМ ВЕТВЯМ, А НЕ ПО ПЕРВОЙ. Ревью воспроизвело запуском, чем плоха
# «первая ветвь»: если два сертификата с одним subject_hash подписаны РАЗНЫМИ корнями, а в
# кандидатах есть только корень первого, прежняя редакция клала оба, шла вверх по первому,
# упиралась в его самоподписанный корень и печатала «цепочка замкнута» с кодом 0 — при том
# что корня той ветви, которая реально подписала лист, в связке нет. Отказ вылезал бы на
# рукопожатии: `unable to get local issuer`. Поэтому ЗАМКНУТОЙ считается цепочка, у которой
# ЗАКРЫТЫ ВСЕ ветви, а незакрытая ветвь называется вслух.
if [[ -n "$want_hash" ]]; then
  walked=1
  queue=("$want_hash")
  seen_hash=""
  ...
  for _ in $(seq 1 8); do
    ...
    cur="${queue[0]}"
    queue=("${queue[@]:1}")
    case " $seen_hash " in *" $cur "*) continue ;; esac
    seen_hash="$seen_hash $cur"
    ...
    if [[ ${#matches[@]} -eq 0 ]]; then
      echo "  ⛌ не нашёл сертификат с subject_hash=$cur среди кандидатов — ветвь не закрыта"
      closed_all=0
      continue
    fi

    # Дедуп по ОТПЕЧАТКУ: один и тот же сертификат публикуется под несколькими именами
    # (проверено 14.08.2026: ruc.cer и ruc2.cer у nces.by побайтово совпадают). Разные же
    # сертификаты с одним именем — это поколения УЦ, и вот их молча выбирать нельзя.
    uniq_file=()
    uniq_fp_list=""
    for m in "${matches[@]}"; do
      fp="$(fp_of "$m")"
      case " $uniq_fp_list " in *" $fp "*) continue ;; esac
      uniq_fp_list="$uniq_fp_list $fp"
      uniq_file+=("$m")
    done
    ...
    if [[ ${#uniq_file[@]} -gt 1 ]]; then
      ...
      for m in "${uniq_file[@]}"; do
        echo "      $(basename "$m") — $(cn_of "$m"), до $(ossl x509 -in "$m" -noout -enddate | sed 's/notAfter=//')"
      done
      echo "    Кладу в связку все и веду вверх КАЖДУЮ ветвь: выбрать молча означало бы угадывать поколение УЦ."
    fi

    for m in "${uniq_file[@]}"; do
      fp="$(fp_of "$m")"
      case " $added_fp " in *" $fp "*) continue ;; esac
      added_fp="$added_fp $fp"
      [[ ${#uniq_file[@]} -eq 1 ]] && echo "  + $(basename "$m") — $(cn_of "$m")"
      cat "$m" >> "$bundle"
      sub="$(ossl x509 -in "$m" -noout -subject_hash)"
      iss="$(ossl x509 -in "$m" -noout -issuer_hash)"
      if [[ "$sub" == "$iss" ]]; then
        echo "  (самоподписанный — ветвь замкнута на корне: $(cn_of "$m"))"
        roots=$((roots+1))
      else
        queue+=("$iss")
      fi
  ...
  [[ ${#queue[@]} -gt 0 ]] && { echo "  ⛌ обход упёрся в ограничение глубины — цепочка длиннее ожидаемой"; closed_all=0; }
  [[ $closed_all -eq 1 && $roots -ge 1 ]] && chain_ok=1
else
...
# ⚠ ДВА РАЗНЫХ СОСТОЯНИЯ, И РАНЬШЕ ОНИ ПЕЧАТАЛИСЬ ОДНОЙ СТРОКОЙ. «Обход не дошёл до корня»
# и «обход не запускался, потому что издатель неизвестен» — разные диагнозы с разными
# действиями, а прежняя редакция на втором случае честно сообщала о ненайденном звене,
# которого никто не искал. Хуже: с включённой живой проверкой вывод мог содержать
# одновременно «✅ СЕРТИФИКАТ БАНКА ПРОВЕРЕН» и «⚠ ЦЕПОЧКА НЕ ЗАМКНУТА».
if [[ $chain_ok -ne 1 ]]; then
  if [[ $walked -eq 0 ]]; then
    echo "  ⚠ ЦЕПОЧКА НЕ СТРОИЛАСЬ: издатель неизвестен (сертификат сервера не получен и"
    echo "    --issuer-hash не задан). В связке лежит всё скачанное подряд — это черновик,"
    echo "    а не проверенный кандидат."
  else
    echo "  ⚠ ЦЕПОЧКА НЕ ЗАМКНУТА: не все ветви дошли до самоподписанного корня. Связка СОБРАНА"
    echo "    ЧАСТИЧНО — в ней не хватает звена, и проверка сертификата банка с ней не пройдёт."
    echo "    Вероятная причина: УЦ опубликовал новый промежуточный сертификат, которого нет в"
    echo "    списке кандидатов."
  fi
  echo
fi
